...
export function useTasks() {
  ...
  const setFilters = useCallback((next) => {
    setSearchParams((current) => {
      ...
      const resetPage = Object.keys(values).some((key) => ['search', 'status', 'priority'].includes(key));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setPagination(response.data?.pagination || { total: 0, page: filters.page, limit: filters.limit, totalPages: 0 });
    } catch (err) {
      setTasks([]);
